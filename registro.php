...
<?php
	...
	if(!empty($_POST))
	{
		...
		//$captcha = $link->real_escape_string($_POST['g-recaptcha-response']);
		
		$activo = 0;
		$tipo_usuario = 2;
		//$secret = 'clave secreta de reCaptcha';
		...
		if(count($errors) == 0)
		{
			...
			//{
				...
				if($registro > 0 )
				{
					...
					$url = 'http://'.$_SERVER["SERVER_NAME"].'/pruebas/sistema/activar.php?id='.$registro.'&val='.$token;
